...
export default function EditItemPage(): React.JSX.Element {
  ...
  useEffect(() => {
    const fetchSpaceDetails = async (): Promise<void> => {
      try {
        const res = await fetch(`/api/spaces/${id}`);
        if (!res.ok) {
          throw new Error("Failed to load spatial record.");
        }
        const data = await res.json();
        setTitle(data.title || "");
        setCategory(data.category || "Japandi Minimalism");
        ...
        setNotification({
          type: "error",
          message: err.message || "Failed to load spatial data.",
        });
      } finally {
        setFetching(false);
      }
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      const result = await updateSpace(id as string, spaceData);

      if (result && result.success) {
        setNotification({
          type: "success",
          message: "Spatial portfolio parameters updated successfully.",
        });
      } else {
        setNotification({
          type: "error",
          message: result?.error || "Failed to update spatial coordinates.",
        });
      }
    } catch (err: any) {
      setNotification({
        type: "error",
        message: err.message || "Failed to submit modifications.",
      });
    } finally {
      setLoading(false);
    }
      ...
      <div className="w-full max-w-4xl space-y-6">
        ...
        >
          <div className="space-y-1">
            ...
            >
              <ArrowLeft className="w-3.5 h-3.5" /> Back to Manage
            </Link>
            <h1 className="text-2xl md:text-3xl font-light uppercase tracking-[0.15em]">
              Edit Spatial Registry
            </h1>
            <p className="text-xs text-neutral-500 font-light">
              Modify the dimensions, visual metrics, or structure coordinates.
            </p>
          </div>
        </motion.div>
